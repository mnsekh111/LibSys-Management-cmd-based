...
BEGIN
  ...
  OPEN c_result FOR 
    SELECT * FROM BOOKED WHERE STATUS = 0 AND START_TIME < SYSDATE - (1/24);
    LOOP
      FETCH c_result INTO my_record;
      EXIT WHEN c_result%NOTFOUND;
      UPDATE BOOKED SET STATUS = 3 WHERE PATRON_ID = my_record.PATRON_ID AND ROOM_NUMBER=my_record.room_number AND START_TIME=my_record.start_time;
    END LOOP;
